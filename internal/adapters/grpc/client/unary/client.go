...
type client struct {
	...
	grpcClient unary.UnaryCalculatorClient
}
...
func (c *client) Open(target string) error {
	...
	c.grpcClient = unary.NewUnaryCalculatorClient(c.grpcConn)
...
func (c *client) Calculate(input models.Calculation) (models.Calculation, error) {
	...
	response, err := c.grpcClient.Calculate(context.Background(), request)
	if err != nil {
		return input, err
	}
